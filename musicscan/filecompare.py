...
def get_torrent_music_file_sizes(torrent_files):
    # return list of file sizes (music/audio only)
    return [ int(torrent_file["size"]) for torrent_file in torrent_files \
                if musicscan.findmusiclocal.is_music_file(torrent_file["name"]) ]
...
def match_torrent_files(torrent_files, release):
    ...
    torrent_music_file_sizes = get_torrent_music_file_sizes(torrent_files)
    logging.debug("torrent_music_file_sizes = %s", torrent_music_file_sizes)
    local_file_sizes = get_local_file_sizes(release["all_files"])
    logging.debug("local_file_sizes = %s", local_file_sizes)
    local_music_file_sizes = get_local_file_sizes(release["music_files"])
    logging.debug("local_music_file_sizes = %s", local_music_file_sizes)
    # get file names
    ...
    # calculate matches
    music_filesize_matches = list(set(torrent_music_file_sizes).intersection(set(local_music_file_sizes)))
    ...
    music_filesize_matches_pct = round(100 * len(music_filesize_matches) / len(torrent_music_file_sizes), 2)
    ...
    matches = {
            "music_filesize_matches_pct": music_filesize_matches_pct,
...
def evaluate_match(matches, music_threshold):
    ...
    if matches["music_filesize_matches_pct"] == 100 and \
